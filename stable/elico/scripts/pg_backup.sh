...
if [ -z $PGBACKUPDIR ]; then
        PGBACKUPDIR="/mnt/backup/postgres"
fi
subdir=`date +%Y-%m`
...
run_b() {
		...
		then
			...
			OF="$PGBACKUPDIR/$subdir-$(date +%d)-pg_db-$PGHOST-$i.backup"
		fi
                ...
                if [ "$backup_bzip2" = "yes" ]; then
                        rm "$OF.bz2"
			bzip2 "$OF"
                        #scp -P 37155 "OF.bz2" "rbackup@42.120.43.36:~/kelit/$subdir-$OF.bz2"
                        chmod $permissions_backup_file "$OF.bz2"
                else
                        #scp -P 37155 "OF.bz2" "rbackup@42.120.43.36:~/kelit/$subdir-$OF"
		...
		then
			...
			OF="$PGBACKUPDIR/$(date +%d)-pg_db-$PGHOST-$i.backup"
                ...
                if [ "$backup_bzip2" = "yes" ]; then
                        rm "$OF.bz2"
			bzip2 "$OF"
			#scp -P 37155 "$OF.bz2" "rbackup@42.120.43.36:~/kelit/$subdir-$(date +%d)-pg_db-$PGHOST-$i.backup.bz2"
                        chmod $permissions_backup_file "$OF.bz2"
                else
			#scp -P 37155 "$OF" "rbackup@42.120.43.36:~/kelit/$subdir-$(date +%d)-pg_db-$PGHOST-$i.backup"
                        chmod $permissions_backup_file "$OF"
                fi
